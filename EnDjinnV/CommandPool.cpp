...
CommandPool::~CommandPool() { FreeDeviceMemory(); }
...
void CommandPool::CreateCommandBuffers(uint32_t count) {
  ...
  vk::Result result =
      vkDevice.allocateCommandBuffers(&commandBufferAllocInfo, buffers.data());
  if (result != vk::Result::eSuccess)
    throw Exception("Unable to allocate command buffer.");
}
