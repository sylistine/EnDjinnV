...
Swapchain::Swapchain(vk::Device device, vk::SurfaceKHR surface,
  ...
  swapchainCI.presentMode =
      vk::PresentModeKHR::eFifo;  // Default, always supported.
  ...
  vk::Result result =
      vkDevice.createSwapchainKHR(&swapchainCI, NULL, &vkSwapchain);
  ...
  for (auto i = 0u; i < swapchainImages.size(); i++) {
    ...
    swapchainImageViewCI.subresourceRange.aspectMask =
    ...
    result = vkDevice.createImageView(&swapchainImageViewCI, NULL,
...
Swapchain::~Swapchain() { FreeDeviceMemory(); }
