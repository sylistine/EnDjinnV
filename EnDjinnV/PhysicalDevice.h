...
class PhysicalDevice {
 ...
 public:
  ...
  size_t QueueFamilyPropertyCount() const {
  ...
  vk::QueueFamilyProperties QueueFamilyProperties(uint32_t idx) const {
    if (idx < 0 || idx >= queueFamilyProperties.size())
  ...
  // `typeBits`, a value returned from the `vkGet*MemoryRequirements` class of
  // functions, determines what indices can be tested. Returns false if no valid
  // index was found.
  bool GetMemoryTypeIndex(uint32_t typeBits, vk::MemoryPropertyFlags flags,
