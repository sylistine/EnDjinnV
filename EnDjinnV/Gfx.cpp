...
void Manager::SetViewProjectionMatrices(mat4 viewMatrix,
                                        mat4 projectionMatrix) {}

void Manager::SetVertices(std::vector<Vertex> vertices) {
  if (gfxInstance == NULL)
    throw Exception("Graphics has not been initialized.");
...
Manager::Manager(vk::Instance vkInstance, vk::SurfaceKHR surface)
  ...
  swapchain =
      Swapchain(device.GetLogical(), surface, primaryGPU.GetOutputFormat(),
  ...
  result = device.GetLogical().createShaderModule(&vsModuleCI, NULL,
                                                  &vertexShaderModule);
  ...
  result = device.GetLogical().createShaderModule(&fsModuleCI, NULL,
...
std::vector<unsigned int> Manager::CompileShader(shaderc::Compiler& compiler,
                                                 Shader shader) {
  auto program = compiler.CompileGlslToSpv(shader.shader, strlen(shader.shader),
  ...
  if (program.GetCompilationStatus() != shaderc_compilation_status_success) {
    ...
    if (program.GetNumErrors() > 0) {
      std::cout << "Detected errors during vertex program compilation."
                << std::endl;
  ...
  for (auto it = program.cbegin(); it != nullptr && it != program.cend();
       it++) {
    data.push_back(*it);
  }
...
Buffer Manager::CreateUniformBuffer(void* data, size_t size) {
  return Buffer(device, vk::BufferUsageFlagBits::eUniformBuffer, data, size);
}
...
void Manager::SetupPrimaryRenderPass() {
  ...
  auto bufferIdx = d.acquireNextImageKHR(swapchain.GetSwapchainKHR(),
                                         UINT64_MAX, semaphore, vk::Fence());
  ...
  vk::Result result =
      d.createRenderPass(&renderpassCI, NULL, &primaryRenderPass);
  ...
  primaryFramebuffer = (vk::Framebuffer*)malloc(swapchain.GetImageCount() *
                                                sizeof(vk::Framebuffer));
...
void Manager::SetPrimaryVertexBuffer(std::vector<Vertex> vertices) {
  Buffer vertexBuffer(device, vk::BufferUsageFlagBits::eVertexBuffer,
