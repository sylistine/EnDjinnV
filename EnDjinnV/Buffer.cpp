...
Buffer::Buffer(const Device& device, vk::BufferUsageFlags usage, void* data,
               VkDeviceSize size)
    : d(device.GetLogical()) {
  ...
  if (!device.GetMemoryTypeIndex(memoryReqs.memoryTypeBits,
                                 vk::MemoryPropertyFlagBits::eHostVisible |
                                     vk::MemoryPropertyFlagBits::eHostCoherent,
                                 memoryInfo.memoryTypeIndex)) {
    d.destroyBuffer(buffer, NULL);
    throw Exception(
        "Unable to locate correct memory type index for vertex buffer.");
  }
  ...
  // Map, Copy, and Unmap
  auto vertexDataDst =
      d.mapMemory(memory, vk::DeviceSize(0), memoryInfo.allocationSize,
                  vk::MemoryMapFlags(0));
