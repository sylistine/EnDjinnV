...
PhysicalDevice::PhysicalDevice(vk::PhysicalDevice dev,
                               VkSurfaceKHR outputSurface)
    : device(dev), outputSurface(outputSurface) {
  vk::Result result;
  ...
  for (auto i = 0u; i < queueFamilyProperties.size(); i++) {
    ...
    if (graphicsFlag == vk::QueueFlagBits::eGraphics &&
      ...
      if (gfxQueueFamilyIdx == UINT32_MAX &&
          graphicsFlag == vk::QueueFlagBits::eGraphics) {
  ...
  result =
      device.getSurfaceCapabilitiesKHR(outputSurface, &surfaceCapabilities);
  if (result != vk::Result::eSuccess) {
    throw Exception(
        "Unable to determine surface capabilities on selected GPU.");
  }
  ...
  if (surfaceFormats.size() < 1)
    throw Exception("Unable to determine surface formats.");
...
bool PhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits,
