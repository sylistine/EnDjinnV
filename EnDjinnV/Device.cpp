...
Device::Device(PhysicalDevice physicalDevice) : physicalDevice(physicalDevice) {
  ...
  vk::Result result =
      vkPhysicalDevice.createDevice(&deviceCI, NULL, &logicalDevice);
  if (result != vk::Result::eSuccess)
    throw Exception("Unable to create logical device.");

  inited = true;
}

Device::~Device() { FreeDeviceMemory(); }
...
bool Djn::Gfx::Device::GetMemoryTypeIndex(uint32_t typeBits,
